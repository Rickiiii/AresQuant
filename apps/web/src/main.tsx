...
const navItems: readonly { readonly key: ViewKey; readonly label: string; readonly icon: React.ReactNode }[] = [
  ...
  { key: 'trading', label: '组合决策', icon: <Radar size={16} /> },
...
const fallbackPortfolioData: PortfolioData = {
  context: {
    ...
    positions: [
      { id: 'fallback-position-600366', symbol: '600366', name: '宁波韵升', quantity: 800, costPrice: '13.470000', latestPrice: null, marketValue: '10776.00', unrealizedPnl: '0.00', dailyPctChange: null, themeTags: ['稀土永磁', '电机材料'], actionBias: 'hold', riskLevel: 'medium' },
      { id: 'fallback-position-601689', symbol: '601689', name: '拓普集团', quantity: 200, costPrice: '69.620000', latestPrice: null, marketValue: '13924.00', unrealizedPnl: '0.00', dailyPctChange: null, themeTags: ['物理AI', '机器人执行器', '智能车'], actionBias: 'hold', riskLevel: 'medium' },
      { id: 'fallback-position-002031', symbol: '002031', name: '巨轮智能', quantity: 2100, costPrice: '8.132900', latestPrice: null, marketValue: '17079.09', unrealizedPnl: '0.00', dailyPctChange: null, themeTags: ['机器人', '工业母机高弹性'], actionBias: 'watch', riskLevel: 'high' },
      { id: 'fallback-position-002714', symbol: '002714', name: '牧原股份', quantity: 100, costPrice: '44.670000', latestPrice: null, marketValue: '4467.00', unrealizedPnl: '0.00', dailyPctChange: null, themeTags: ['生猪养殖', '农业'], actionBias: 'hold', riskLevel: 'medium' },
      { id: 'fallback-position-603005', symbol: '603005', name: '晶方科技', quantity: 200, costPrice: '38.397000', latestPrice: null, marketValue: '7679.40', unrealizedPnl: '0.00', dailyPctChange: null, themeTags: ['半导体封测', 'CIS封装'], actionBias: 'watch', riskLevel: 'medium' },
    ],
  ...
  tradingDecision: {
    ...
    summary: {
      ...
      totalMarketValue: '59925.49',
      totalUnrealizedPnl: '2160.30',
      totalUnrealizedPnlPercent: '3.74',
  ...
  return (
    <div className="view-stack">
      <section className="hero-card trading-hero">
        <div>
          <div className="eyebrow"><Radar size={14} /> {props.isLive ? '组合接口已连接' : '预览组合决策'} · {context.source === 'database' ? '数据库组合' : 'Fallback 组合'}</div>
          <h2>把你的持仓翻译成今日可执行的观察、持有和风控队列</h2>
          <p>{decision.summary.primaryAction}</p>
          <div className="hero-actions">
            <button className="primary-action" onClick={() => void props.onRefresh()} type="button"><RefreshCcw size={16} /> 刷新组合决策</button>
          </div>
        </div>
        <div className="decision-score">
          <span>市场状态</span>
          <strong>{decision.marketRegime.label}</strong>
          <small>风险 {riskLevelLabel(decision.summary.riskLevel)} · 分数 {decision.marketRegime.score}</small>
        </div>
      </section>
      ...
      <section className="trading-command-grid">
      ...
      <section className="trading-plan-grid">
        {actionRows.map((row) => (
      ...
      <article className="glass-card position-decision-table">
        <CardHeader icon={<Table2 />} title="持仓触发矩阵" subtitle="逐只看动作、理由和价格触发" />
        <div className="table-head">
          <span>标的</span><span>动作</span><span>当前/成本</span><span>风控线</span><span>理由</span>
        </div>
        {decisions.map((item) => (
          <div className="table-row decision-row" key={item.symbol}>
            <span className="decision-target"><strong>{item.name}</strong><small>{getDecisionTheme(item, themeTagsBySymbol)}</small></span>
            <span className={`decision-pill ${item.action}`}>{item.actionLabel}</span>
            <span className="decision-price">{props.showAmounts ? `${item.latestPrice ?? '--'} / ${item.costPrice}` : '金额已隐藏'}</span>
            <span className="decision-price">{props.showAmounts ? item.pricePlan.stopLossPrice ?? '--' : '已隐藏'}</span>
            <span className="decision-reason">{item.reasons[0] ?? '等待补充逻辑'}</span>
          </div>
        ))}
      </article>
