...
AresQuant 是一个基于 NestJS + Prisma 的 A 股量化交易系统。当前系统已经具备 A 股数据中心、Mock 数据源、东方财富真实数据源基础接入、数据质量校验、复权价格计算、专业回测引擎、多因子策略系统、正式策略回测接入、Web Dashboard、Research Center，以及 Portfolio Context 持仓上下文基础能力。
...
精简使用说明：[docs/user-manual.md](docs/user-manual.md)
...
- 从 `.env.example` 创建 `.env`（如果还没有）
- 启动 Docker Postgres / Redis
- 执行 Prisma generate / migrate deploy
- 启动 NestJS API
- 写入 Ricki Portfolio Context 种子数据
- 启动 Web Dashboard
...
协作约定：后续新增功能、页面文案、使用说明和需求描述默认使用中文；专业名词、代码标识、接口路径、策略 code、技术缩写和必要英文品牌名保留英文。
...
Phase 9 已加入 `ResearchModule` 第一版只读 API，并在后续迭代中接入 Dashboard / Strategy / Backtest / Portfolio Context 数据；当后端数据不可用时仍保留 fallback 预览：
...
Phase 10 Step 1 已加入 `PortfolioModule` 和持仓上下文基础模型：
...
- `ThemeExposure`
- `WatchlistItem`
- `GET /portfolio/context`：读取 Ricki 持仓、基金暴露、主题矩阵、观察主题、风险旗标和仓位汇总
- `POST /portfolio/context/seed-ricki`：把当前 Ricki 持仓 / 基金暴露 / 主题矩阵种子数据幂等写入 Portfolio 表

Research Center 会优先读取 Portfolio Context；没有持久化持仓数据时继续回退到原静态上下文。
